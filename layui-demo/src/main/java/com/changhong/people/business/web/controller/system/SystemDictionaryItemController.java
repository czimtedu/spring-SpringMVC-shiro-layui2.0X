...
import com.changhong.people.business.entity.system.SystemDictionaryItem;
import com.changhong.people.business.query.system.SystemDictionaryItemQuery;
import com.changhong.people.business.service.system.ISystemDictionaryItemService;
...
public class SystemDictionaryItemController extends BaseController {
	...
	public AjaxResult save(SystemDictionaryItem systemDictionaryItem) {
		...
		try {
			...
			LogUtils.writeOperateLog("数据字典", "{}账号:{},新增code为:[{}]的明细,唯一标识[{}]", user.getName(),user.getAccount(),systemDictionaryItem.getCode(),systemDictionaryItem.getValue());
		} catch (Exception e) {
			e.printStackTrace();
	...
	public AjaxResult edit(SystemDictionaryItem systemDictionaryItem) {
		...
		try {
			...
			SysUser user = ShiroUtils.getUserInSession();
			LogUtils.writeOperateLog("数据字典", "{}账号:{},编辑code为:[{}]的明细,唯一标识[{}]", user.getName(),user.getAccount(),systemDictionaryItem.getCode(),systemDictionaryItem.getValue());
		} catch (Exception e) {
			e.printStackTrace();
	...
	public AjaxResult delete(@PathVariable("id")Long id) {
		...
		try {
			...
			SysUser user = ShiroUtils.getUserInSession();
			LogUtils.writeOperateLog("数据字典", "{}账号:{},删除code为[{}],ID为[{}],value为[{}],字典明细", user.getName(),user.getAccount(),systemDictionaryItem.getCode(),id,systemDictionaryItem.getValue());
		} catch (Exception e) {
			e.printStackTrace();
