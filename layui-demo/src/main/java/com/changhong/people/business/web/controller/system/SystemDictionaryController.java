...
import com.changhong.people.business.query.system.SystemDictionaryQuery;
...
public class SystemDictionaryController extends BaseController{
	...
	public AjaxResult add(SystemDictionary systemDictionary) {
		...
		try {
			...
			LogUtils.writeOperateLog("数据字典", "{}账号:{},新增code为:[{}]的字典", user.getName(),user.getAccount(),systemDictionary.getCode());
		} catch (Exception e) {
			e.printStackTrace();
	...
	public AjaxResult edit(SystemDictionary systemDictionary) {
		...
		try {
			...
			ajaxResult = new AjaxResult(true, "修改成功");
			SysUser user = ShiroUtils.getUserInSession();
			LogUtils.writeOperateLog("数据字典", "{}账号:{},编辑code为:[{}]的字典", user.getName(),user.getAccount(),systemDictionary.getCode());
		} catch (Exception e) {
			e.printStackTrace();
			ajaxResult = new AjaxResult(false, "修改失败");
		}
