...
public class LogUtils {
	...
	public static void writeOperateLog( String operatAddress ,String contentFormate , Object... arguments) throws Exception{
		try {
			SysUser userInSession = ShiroUtils.getUserInSession();
			Long adminId = userInSession.getId();
			
			getOpLogService().save(new OpLog(adminId,operatAddress,format(contentFormate,arguments)));
		} catch (Exception e) {
	...
	private static IOpLogService getOpLogService(){
		if(null == opLogService){
			...
			opLogService = (IOpLogService)Const.WEB_APP_CONTEXT.getBean("opLogServiceImpl");
		}
		return  opLogService ;
