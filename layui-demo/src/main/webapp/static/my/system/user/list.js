layui.use('table', function() {
	...
	parent.tableRender = table.render({
		...
		cols : [ [ {
			...
			title : '状态',
			width : 60,
			templet : '<div>{{d.status==0 ? "启用": "禁用"}}</div>'
		}, {
			unresize : true,//列宽是否禁止拖动(默认  false)
			field : 'createTime',
			title : '创建时间',
	...
	table.on('tool(user)', function(obj) {
		...
		if (obj.event === 'detail') {
			parent.layer_form = layer_form = parent.layer.open({
				...
				type : 2,
				area : [ "700px", "85%" ],
				content : rootPath + '/sysuser/getDetails?id=' + data.id
			});
			...
			parent.layer_form = layer_form = parent.layer.open({
				...
				type : 2,
				area : [ "600px", "80%" ],
				content : rootPath + '/sysuser/editUI/' + data.id
			});
			...
			parent.layer_form = layer_form = parent.layer.open({
				...
				type : 2,
				area : [ "550px", "330px" ],
				content : rootPath + '/sysuser/resetPassUI?id=' + data.id
			...
			parent.layer_form = layer_form = parent.layer.open({
				...
				type : 2,
				area : [ "550px", "330px" ],
				content : rootPath + '/sysuser/updateStatusUI?id=' + data.id
						+ '&isEnabled=' + isEnabled
			});
		}
	});
...
function toAdd() {
	parent.layer_form = layer_form = parent.layer.open({
		...
		type : 2,
		area : [ "600px", "80%" ],
		content : rootPath + '/sysuser/addUI'
	});
}

/* 显示导出下载列表 */
function toExportView() {
	var viewUrl = rootPath + "/sysuser/getCount";
	var downUrl = '/sysuser/export';
